...
```{r}
#Getting comfortable with 2024 candidate data and formatting in useful way

candidates2024 <- read.csv("/Users/liam/Downloads/courses/PLSC_204/ISPS Election Prediction/2024_candidates.csv")

#removing presidential candidate
#candidates2024 <- subset(candidates2024, !Cand_Office == "P")

#candidates2024_house <- subset(candidates2024, Cand_Office == "H")
#aggregate(candidates2024_house$Cand_Incumbent_Challenger_Open_Seat,list(candidates2024_house$CD), mean)

#candidates2024_house_collapsed <- candidates2024_house %>%
#  group_by(CD) %>%
#    mutate(incumbency = Cand_Incumbent_Challenger_Open_Seat)

#candidates2024_house <- subset(candidates2024_house, Cand_Party_Affiliation %in% c("DEM", "REP"))
#candidates2024_house <- candidates2024_house[,!colnames(candidates2024_house) %in% c("Cand_Id", "Cand_Office_St", "Cand_Office_Dist", "Cand_Office")] 
#house2024 <- reshape(candidates2024_house, idvar = "CD", timevar = "Cand_Party_Affiliation", direction = "wide")
#house2024 <- house2024[order(house2024$CD),]

#candidates2024_senate <- subset(candidates2024, Cand_Office == "S")
#reshaping 
...
2024 election data
```{r}
houseraces2024 <- read.csv("/Users/liam/Downloads/2024houseraces.csv")

#census_oneyear$district <- str_split_fixed(census_oneyear$name, " ", 2)[,2]
...
senateraces2024 <- read.csv("/Users/liam/Downloads/2024senateraces.csv")
